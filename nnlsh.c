...
int nnlsh(char * input, char * output,int val)
{
	...
	{
		if(choice==0)
			hamminglsh(hasht,finalmatrix,idnum,prodnum,suser,output,l,k,matrix,val,&error);
		else if(choice==1)
			cosinelsh(hasht,finalmatrix,idnum,prodnum,suser,output,l,k,matrix,val,&error);
		else if(choice==2)
			euclideanlsh(hasht,finalmatrix,idnum,prodnum,suser,output,l,k,matrix,val,&error);
	}
	else
	{
		error=0;
...
int hamminglsh(struct hashtable * hasht,double **matrix,int idnum,int prodnum,struct user * suser,char * output,int L,int k,int **matrix2,int val,double *error)
{
	...
	if(val==0)
		findbestlsh(bestid,matrix, L,idnum,prodnum, P,suser, 0,matrix2,output);
	else
	...
	for(i=0;i<idnum;i++)
		for(j=0;j<P;j++)
			if(nearest[0][i][j]>0 && nearest[0][i][j]<=idnum )
				...
				bestid[i][j]=rand()%idnum+1;
	if(val==0)
		findbestlsh(bestid,matrix, L,idnum,prodnum, P,suser, 1,matrix2,output);
	else
...
int euclideanlsh(struct hashtable * hasht,double **matrix,int idnum,int prodnum,struct user * suser,char * output,int L,int k,int **matrix2,int val,double *error)
{
	...
	for(m=0;m<idnum;m++)
	{
		...
		{	
			...
			insertht(&hasht[i].lista[pos],m+1);
		}
	}
	...
	for(i=0;i<idnum;i++)
		for(j=0;j<P;j++)
			if(nearest[0][i][j]>0 && nearest[0][i][j]<=idnum )
				...
				bestid[i][j]=rand()%idnum+1;
	if(val==0)
		findbestlsh(bestid,matrix, L,idnum,prodnum, P,suser, 2,matrix2,output);
	else
