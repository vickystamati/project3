...
int clusrec(char* filename,char*output,int val)
{
	...
	if(val==0)
		assert(clusteringmethod(matrix,finalmatrix,prodnum,idnum,suser,output,val,&error,choice));
	else
	{
		error=0;
		for(i=0;i<1;i++)
...
int clusteringmethod(int ** matrix,double ** finalmatrix,int productnum,int idnum,struct user * suser,char * output,int val,double*error,int choice)
{
	...
	if(val==0)
		assert(findbest(clistfinal,finalmatrix,suser,productnum,idnum,choice,bestcent,matrix,output));
	else
	...
	for(i=0;i<s;i++)
	{
		...
		else
		{
			...
			if(jsum<jsumclara)
			{
				...
				assert(swapclistnokey(clist,clistptr,cent));
			}
			assert(freeclustlistnokey(clistptr,cent));
		}
	}
...
int clarafclu(struct clustlist * clist , int ** matrix,double **finalmatrix,int productnum,int idnum,int cent,struct user * suser)
{
	...
	assert(medpamclu(matrix,finalmatrix,clist,productnum,idnum,cent,listmatr,nnum,suser));
	...
	freeclustlistnokey(clistfinal,cent);
	freeclustlistnokey(clist,cent);
	assert(medpamclu(matrix,finalmatrix,clist,productnum,idnum,cent,listmatr,0,suser));
	free(clistfinal);
	free(clisttemp);
	free(listmatr);
	return 1;
...
int medpamclu(int **matrix,double ** finalmatrix,struct clustlist * clist,int productnum,int idnum,int cent,int *matr,int nnum,struct user * suser)
{
	...
	for(k=0;k<loop;k++)
	{
		...
		if(flag==1)
		{
			...
			if(nnum==0)
				assert(insertassigclu(pammatr,clist,k+1,suser));	
			else
				assert(insertassigclu(pammatr,clist,matr[k],suser));	
		}
	}
	for(i=0;i<cent;i++)
		free(pammatr[i]);
